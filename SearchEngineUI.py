...
import webCrawlerStuff

seed_url = "https://google.com/search?q="
...
def perform_search():
    ...
    result_title_list, paragraph_list = webCrawlerStuff.initialise(seed_url + query)

    results_text.config(state=tk.NORMAL)
    #results_text.delete("1.0", tk.END)
    for i in range(len(result_title_list)):
        results_text.insert(tk.END, result_title_list[i] + "\n" + paragraph_list[i] + "\n\n")
